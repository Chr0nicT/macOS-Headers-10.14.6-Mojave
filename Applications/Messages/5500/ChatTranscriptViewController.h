//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Jun  9 2015 22:53:21).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2014 by Steve Nygard.
//

#import <SocialUI/SOTranscriptViewController.h>

#import "AVAudioPlayerDelegate-Protocol.h"
#import "BCMessageHandlerDelegate-Protocol.h"
#import "ChatTranscriptViewAcknowledgmentPickerHelperDelegate-Protocol.h"
#import "FezWebViewDragDelegate-Protocol.h"
#import "FezWebViewPasteboardDelegate-Protocol.h"
#import "FezWebViewQuickLookDelegate-Protocol.h"
#import "FezWebViewViewDelegate-Protocol.h"
#import "NSAnimationDelegate-Protocol.h"
#import "QLPreviewPanelDataSource-Protocol.h"
#import "QLPreviewPanelDelegate-Protocol.h"
#import "SOBalloonPluginCache-Protocol.h"
#import "SOFaceTimeCallHandlerDelegate-Protocol.h"
#import "TranscriptDOMControllerDelegate-Protocol.h"
#import "WebEditingDelegate-Protocol.h"
#import "WebFrameLoadDelegate-Protocol.h"
#import "WebPolicyDelegate-Protocol.h"
#import "WebResourceLoadDelegate-Protocol.h"
#import "WebUIDelegate-Protocol.h"

@class AVAudioPlayer, Animator, BCMessageHandler, BuddyCSSProvider, ChatController, ChatTranscriptViewAcknowledgmentPickerHelper, ChatTranscriptViewDragDestinationHelper, DOMElement, DOMRange, FezWebView, IMGroupAcknowledgementPollViewController, IMMessage, IMMessagePartChatItem, IMTranscriptChatItem, NSArray, NSLayoutConstraint, NSMenuItem, NSMutableArray, NSMutableDictionary, NSMutableSet, NSString, NSTimer, ProgressAnimation, QLPreviewPanel, SODynamicLayoutManager, SOFaceTimeCallHandler, SOFullScreenMomentQueue, SOTypingIndicatorView, ScrollViewWatcher, TranscriptDOMController, TranscriptStyleController, WebKitTranscriptFirstResponder;

@interface ChatTranscriptViewController : SOTranscriptViewController <ChatTranscriptViewAcknowledgmentPickerHelperDelegate, WebUIDelegate, WebPolicyDelegate, WebFrameLoadDelegate, WebResourceLoadDelegate, WebEditingDelegate, BCMessageHandlerDelegate, SOFaceTimeCallHandlerDelegate, TranscriptDOMControllerDelegate, FezWebViewViewDelegate, FezWebViewDragDelegate, FezWebViewQuickLookDelegate, FezWebViewPasteboardDelegate, SOBalloonPluginCache, AVAudioPlayerDelegate, NSAnimationDelegate, QLPreviewPanelDataSource, QLPreviewPanelDelegate>
{
    NSMutableArray *_selectedMessageMenuItems;
    NSMutableArray *_selectedUnsavedMessageMenuItems;
    NSMutableArray *_acknowledgmentMenuItems;
    NSMenuItem *_quickLookTransferMenuItem;
    ChatTranscriptViewDragDestinationHelper *_dragHelper;
    NSMutableSet *_highlightedItems;
    BOOL _pinToBottomOnResize;
    BOOL _pinToContentOnChange;
    BOOL _needsNewLayout;
    BOOL _isForcingRelayout;
    BOOL _isLoading;
    BOOL _canShowMessages;
    BOOL _aboutBlankHandled;
    BOOL _delayScrolling;
    BOOL _handingKeyEvent;
    BOOL _unreadCountChanged;
    NSMutableArray *_loadingIdentifiers;
    int _changeCount;
    int _changeCountForTranscriptStyleController;
    double _heightBeforeChange;
    struct CGRect _clipRectBeforeChange;
    NSMutableArray *_changeCallStacks;
    NSTimer *_changeTimeoutTimer;
    SOFullScreenMomentQueue *_fullScreenMomentQueue;
    BOOL _shouldAnimatePinAfterChange;
    BOOL _isWaitingForAcknowledgmentMouseup;
    BOOL _isDisplayingGroupAcknowledgmentPoll;
    BOOL _hasHitBoundsLimit;
    BOOL _isHandlingTick;
    BOOL _isHandlingLiveScroll;
    BOOL _createStickerContextMenuOnly;
    BOOL _updatingTypingIndicator;
    SOFaceTimeCallHandler *_faceTimeCallHandler;
    NSMutableDictionary *_balloonPluginCache;
    NSMutableDictionary *_balloonPluginDataSourceCache;
    NSMutableDictionary *_balloonPluginControllerCache;
    IMTranscriptChatItem *_pinnedToChatItem;
    TranscriptStyleController *_transcriptStyleController;
    TranscriptDOMController *_transcriptDOMController;
    BuddyCSSProvider *_buddyCSSProvider;
    QLPreviewPanel *_qlPreviewPanel;
    long long _qlStartIndex;
    NSArray *_qlAttachments;
    ChatTranscriptViewAcknowledgmentPickerHelper *_acknowledgmentHelper;
    DOMElement *_acknowledgmentPickerElement;
    IMGroupAcknowledgementPollViewController *_groupAcknowledgementPollViewController;
    ScrollViewWatcher *_scrollViewWatcher;
    Animator *_currentScrollingAnimation;
    double _startingScrollingValue;
    double _endingScrollingValue;
    NSMutableSet *_expiredTransferGUIDs;
    NSMutableSet *_deferredContactInfoUpdates;
    NSMutableSet *_deferredContactPictureUpdates;
    NSMutableDictionary *_messageGUIDServices;
    IMMessage *_lastClickedMessage;
    NSString *_lastRightClickedTransferGUID;
    BCMessageHandler *_lastClickedBizMessageHandler;
    IMMessagePartChatItem *_delayedMessageSelection;
    AVAudioPlayer *_audioMessagePlayer;
    NSMutableArray *_audioMessagePlayingChatItems;
    DOMElement *_audioMessagePlayedWaveformElement;
    DOMElement *_audioMessageTimeElement;
    DOMElement *_audioMessageProgressElement;
    DOMElement *_audioMessageButton;
    DOMRange *_selectedRangeBeforeLongClick;
    SODynamicLayoutManager *_dynamicLayoutManager;
    double _lastDistanceFromBottom;
    ProgressAnimation *_dynamicLayoutAnimation;
    NSMutableSet *_previousVisibleElements;
    SOTypingIndicatorView *_typingView;
    NSLayoutConstraint *_typingIndicatorXConstraint;
    NSLayoutConstraint *_typingIndicatorYConstraint;
    WebKitTranscriptFirstResponder *_transcriptFirstResponder;
}

+ (id)_defaultViewMenu;
+ (void)initialize;
@property(nonatomic) BOOL updatingTypingIndicator; // @synthesize updatingTypingIndicator=_updatingTypingIndicator;
@property(nonatomic) BOOL createStickerContextMenuOnly; // @synthesize createStickerContextMenuOnly=_createStickerContextMenuOnly;
@property(readonly) WebKitTranscriptFirstResponder *transcriptFirstResponder; // @synthesize transcriptFirstResponder=_transcriptFirstResponder;
@property(retain, nonatomic) NSLayoutConstraint *typingIndicatorYConstraint; // @synthesize typingIndicatorYConstraint=_typingIndicatorYConstraint;
@property(retain, nonatomic) NSLayoutConstraint *typingIndicatorXConstraint; // @synthesize typingIndicatorXConstraint=_typingIndicatorXConstraint;
@property(retain, nonatomic) SOTypingIndicatorView *typingView; // @synthesize typingView=_typingView;
@property(nonatomic) BOOL isHandlingLiveScroll; // @synthesize isHandlingLiveScroll=_isHandlingLiveScroll;
@property(nonatomic) BOOL isHandlingTick; // @synthesize isHandlingTick=_isHandlingTick;
@property(retain, nonatomic) NSMutableSet *previousVisibleElements; // @synthesize previousVisibleElements=_previousVisibleElements;
@property(retain, nonatomic) ProgressAnimation *dynamicLayoutAnimation; // @synthesize dynamicLayoutAnimation=_dynamicLayoutAnimation;
@property(nonatomic) double lastDistanceFromBottom; // @synthesize lastDistanceFromBottom=_lastDistanceFromBottom;
@property(retain, nonatomic) SODynamicLayoutManager *dynamicLayoutManager; // @synthesize dynamicLayoutManager=_dynamicLayoutManager;
@property(retain, nonatomic) DOMRange *selectedRangeBeforeLongClick; // @synthesize selectedRangeBeforeLongClick=_selectedRangeBeforeLongClick;
@property(retain, nonatomic) DOMElement *audioMessageButton; // @synthesize audioMessageButton=_audioMessageButton;
@property(retain, nonatomic) DOMElement *audioMessageProgressElement; // @synthesize audioMessageProgressElement=_audioMessageProgressElement;
@property(retain, nonatomic) DOMElement *audioMessageTimeElement; // @synthesize audioMessageTimeElement=_audioMessageTimeElement;
@property(retain, nonatomic) DOMElement *audioMessagePlayedWaveformElement; // @synthesize audioMessagePlayedWaveformElement=_audioMessagePlayedWaveformElement;
@property(retain, nonatomic) NSMutableArray *audioMessagePlayingChatItems; // @synthesize audioMessagePlayingChatItems=_audioMessagePlayingChatItems;
@property(retain, nonatomic) AVAudioPlayer *audioMessagePlayer; // @synthesize audioMessagePlayer=_audioMessagePlayer;
@property(retain, nonatomic) IMMessagePartChatItem *delayedMessageSelection; // @synthesize delayedMessageSelection=_delayedMessageSelection;
@property(retain, nonatomic) BCMessageHandler *lastClickedBizMessageHandler; // @synthesize lastClickedBizMessageHandler=_lastClickedBizMessageHandler;
@property(retain, nonatomic) NSString *lastRightClickedTransferGUID; // @synthesize lastRightClickedTransferGUID=_lastRightClickedTransferGUID;
@property(retain, nonatomic) IMMessage *lastClickedMessage; // @synthesize lastClickedMessage=_lastClickedMessage;
@property(retain) NSMutableDictionary *messageGUIDServices; // @synthesize messageGUIDServices=_messageGUIDServices;
@property(retain) NSMutableSet *deferredContactPictureUpdates; // @synthesize deferredContactPictureUpdates=_deferredContactPictureUpdates;
@property(retain) NSMutableSet *deferredContactInfoUpdates; // @synthesize deferredContactInfoUpdates=_deferredContactInfoUpdates;
@property(retain) NSMutableSet *expiredTransferGUIDs; // @synthesize expiredTransferGUIDs=_expiredTransferGUIDs;
@property BOOL hasHitBoundsLimit; // @synthesize hasHitBoundsLimit=_hasHitBoundsLimit;
@property double endingScrollingValue; // @synthesize endingScrollingValue=_endingScrollingValue;
@property double startingScrollingValue; // @synthesize startingScrollingValue=_startingScrollingValue;
@property(retain) Animator *currentScrollingAnimation; // @synthesize currentScrollingAnimation=_currentScrollingAnimation;
@property(retain) ScrollViewWatcher *scrollViewWatcher; // @synthesize scrollViewWatcher=_scrollViewWatcher;
@property(retain, nonatomic) IMGroupAcknowledgementPollViewController *groupAcknowledgementPollViewController; // @synthesize groupAcknowledgementPollViewController=_groupAcknowledgementPollViewController;
@property(retain, nonatomic) DOMElement *acknowledgmentPickerElement; // @synthesize acknowledgmentPickerElement=_acknowledgmentPickerElement;
@property(retain, nonatomic) ChatTranscriptViewAcknowledgmentPickerHelper *acknowledgmentHelper; // @synthesize acknowledgmentHelper=_acknowledgmentHelper;
@property(nonatomic, setter=setDisplayingGroupAcknowledgmentPoll:) BOOL isDisplayingGroupAcknowledgmentPoll; // @synthesize isDisplayingGroupAcknowledgmentPoll=_isDisplayingGroupAcknowledgmentPoll;
@property(nonatomic, setter=setWaitingForAcknowledgmentMouseup:) BOOL isWaitingForAcknowledgmentMouseup; // @synthesize isWaitingForAcknowledgmentMouseup=_isWaitingForAcknowledgmentMouseup;
@property(retain, nonatomic, setter=setQLAttachments:) NSArray *qlAttachments; // @synthesize qlAttachments=_qlAttachments;
@property(nonatomic, setter=setQLStartIndex:) long long qlStartIndex; // @synthesize qlStartIndex=_qlStartIndex;
@property(retain, setter=setQLPreviewPanel:) QLPreviewPanel *qlPreviewPanel; // @synthesize qlPreviewPanel=_qlPreviewPanel;
@property(retain, nonatomic) BuddyCSSProvider *buddyCSSProvider; // @synthesize buddyCSSProvider=_buddyCSSProvider;
@property(retain, nonatomic) TranscriptDOMController *transcriptDOMController; // @synthesize transcriptDOMController=_transcriptDOMController;
@property(retain, nonatomic) TranscriptStyleController *transcriptStyleController; // @synthesize transcriptStyleController=_transcriptStyleController;
@property(nonatomic) BOOL shouldAnimatePinAfterChange; // @synthesize shouldAnimatePinAfterChange=_shouldAnimatePinAfterChange;
@property(retain, nonatomic) IMTranscriptChatItem *pinnedToChatItem; // @synthesize pinnedToChatItem=_pinnedToChatItem;
@property(retain, nonatomic) SOFaceTimeCallHandler *faceTimeCallHandler; // @synthesize faceTimeCallHandler=_faceTimeCallHandler;
- (void).cxx_destruct;
- (id)transcriptDOMControllerEffectiveAppearance:(id)arg1;
- (void)_appearanceChanged:(id)arg1;
- (void)messageHandler:(id)arg1 sendMessage:(id)arg2;
- (void)messageHandler:(id)arg1 presentationPropertiesDidChange:(id)arg2;
- (id)initWithNibName:(id)arg1 bundle:(id)arg2;
- (void)viewWillDisappear;
- (void)viewDidAppear;
- (void)viewDidLayout;
- (void)viewDidLoad;
- (void)chatDisplayControllerDidChange:(id)arg1;
- (void)chatDisplayControllerWillChange:(id)arg1;
- (void)updateQLPreviewPanel:(id)arg1;
- (void)copySelectionIntoPasteboard:(id)arg1;
- (void)deselectMessage:(id)arg1;
- (void)selectMessage:(id)arg1;
- (void)deselectAll;
- (void)chatDidChange:(id)arg1;
- (void)chatWillChange:(id)arg1;
- (void)audioPlayerDidFinishPlaying:(id)arg1 successfully:(BOOL)arg2;
- (void)didFinishPlayingAudio;
- (void)stopPlayingAudio;
- (void)updatePlayProgress;
- (void)startPlayingAudioForChatItem:(id)arg1;
- (void)insertTextFromSelectedMessageToPasteboard:(id)arg1;
- (BOOL)hasSelectedMessages;
- (BOOL)fezWebView:(id)arg1 quickLookWithEvent:(id)arg2;
- (void)webView:(id)arg1 decidePolicyForNewWindowAction:(id)arg2 request:(id)arg3 newFrameName:(id)arg4 decisionListener:(id)arg5;
- (void)webView:(id)arg1 decidePolicyForNavigationAction:(id)arg2 request:(id)arg3 frame:(id)arg4 decisionListener:(id)arg5;
- (struct CGRect)previewPanel:(id)arg1 sourceFrameOnScreenForPreviewItem:(id)arg2;
- (id)previewPanel:(id)arg1 previewItemAtIndex:(long long)arg2;
- (long long)numberOfPreviewItemsInPreviewPanel:(id)arg1;
@property(readonly, nonatomic) NSMutableDictionary *balloonPluginControllerCache; // @synthesize balloonPluginControllerCache=_balloonPluginControllerCache;
@property(readonly, nonatomic) NSMutableDictionary *balloonPluginDataSourceCache; // @synthesize balloonPluginDataSourceCache=_balloonPluginDataSourceCache;
@property(readonly, nonatomic) NSMutableDictionary *balloonPluginCache; // @synthesize balloonPluginCache=_balloonPluginCache;
- (void)setBalloonPlugin:(id)arg1 controller:(id)arg2 dataSource:(id)arg3 forGUID:(id)arg4;
- (id)balloonPluginControllerForGUID:(id)arg1;
- (id)balloonPluginForGUID:(id)arg1;
- (id)dataSourceForGUID:(id)arg1;
- (void)fezWebView:(id)arg1 concludeDragOperation:(id)arg2;
- (BOOL)fezWebView:(id)arg1 performDragOperation:(id)arg2;
- (BOOL)fezWebView:(id)arg1 prepareForDragOperation:(id)arg2;
- (void)fezWebView:(id)arg1 draggingExited:(id)arg2;
- (unsigned long long)fezWebView:(id)arg1 draggingUpdated:(id)arg2;
- (unsigned long long)fezWebView:(id)arg1 draggingEntered:(id)arg2;
- (void)fezWebViewShouldRegisterForDraggedTypes:(id)arg1;
- (void)deleteMessages:(id)arg1;
- (void)delete:(id)arg1;
- (void)copy:(id)arg1;
- (void)fezWebViewDidEndLiveResize:(id)arg1;
- (void)fezWebViewWillStartLiveResize:(id)arg1;
- (void)fezWebView:(id)arg1 setFrameSize:(struct CGSize)arg2;
- (id)_webView:(id)arg1 actionContextForHitTestResult:(id)arg2 range:(id *)arg3;
- (BOOL)webView:(id)arg1 validateUserInterfaceItem:(id)arg2 defaultValidation:(BOOL)arg3;
- (id)webView:(id)arg1 contextMenuItemsForElement:(id)arg2 defaultMenuItems:(id)arg3;
- (id)setUpAcknowledgmentAndStickerMenuItems:(BOOL)arg1 chatItemGuid:(id)arg2;
- (void)_setUpDefaultMenuItems:(id)arg1 messageElement:(id)arg2 result:(id)arg3;
- (void)_copyForDraggingMedia:(id)arg1 toPasteboard:(id)arg2;
- (void)webView:(id)arg1 willPerformDragSourceAction:(unsigned long long)arg2 fromPoint:(struct CGPoint)arg3 withPasteboard:(id)arg4;
- (unsigned long long)webView:(id)arg1 dragSourceActionMaskForPoint:(struct CGPoint)arg2;
- (BOOL)writeBalloonPluginItemsAndSnapshotElementToPboard:(id)arg1 forElement:(id)arg2;
- (BOOL)_appendBalloonPluginItems:(id)arg1 itemsToPaste:(id)arg2 messageGuid:(id)arg3 element:(id)arg4;
- (void)_appendURLsForRichLinks:(id)arg1 itemsToPaste:(id)arg2;
- (BOOL)writeBalloonPluginItemsAndUseImageSourceToPboard:(id)arg1 forElement:(id)arg2;
- (void)_createItemsAndWriteToPasteboard:(id)arg1 imageSource:(id)arg2 balloonPluginChatItem:(id)arg3;
- (BOOL)webView:(id)arg1 doCommandBySelector:(SEL)arg2;
- (void)webView:(id)arg1 didLayout:(unsigned long long)arg2;
- (void)webView:(id)arg1 didFinishLoadForFrame:(id)arg2;
- (void)webView:(id)arg1 resource:(id)arg2 didFinishLoadingFromDataSource:(id)arg3;
- (void)webView:(id)arg1 resource:(id)arg2 didFailLoadingWithError:(id)arg3 fromDataSource:(id)arg4;
- (id)webView:(id)arg1 resource:(id)arg2 willSendRequest:(id)arg3 redirectResponse:(id)arg4 fromDataSource:(id)arg5;
- (id)webView:(id)arg1 identifierForInitialRequest:(id)arg2 fromDataSource:(id)arg3;
- (void)_decreaseResourceLoadCountForURL:(id)arg1;
- (void)_increaseResourceLoadCountForURL:(id)arg1;
- (void)_handleInfoChangedNotification:(id)arg1;
- (void)_handlePictureChangedNotification:(id)arg1;
- (void)_updateDeferredHandles;
- (void)_fileTransferUpdated:(id)arg1;
- (void)_transferResourceLoaded:(id)arg1;
- (id)_getIMServiceIMplForMessageGUID:(id)arg1;
- (void)_chatDisplayControllerDisplayedSearchResultDidChange:(id)arg1;
- (void)_chatDisplayControllerDidStopDisplayingSearchResults:(id)arg1;
- (void)makeTextStandardSize:(id)arg1;
- (BOOL)canMakeTextStandardSize;
- (void)makeTextSmaller:(id)arg1;
- (BOOL)canMakeTextSmaller;
- (void)makeTextLarger:(id)arg1;
- (BOOL)canMakeTextLarger;
- (void)copySelectionToFindPboard:(id)arg1;
- (BOOL)validateMenuItem:(id)arg1;
- (void)_acknowledgmentForcedown:(id)arg1;
- (void)_transcriptAcknowledgmentCancelLongClick:(id)arg1;
- (void)_cancelAcknowledgmentLongClick;
- (void)_acknowledgmentMouseup:(id)arg1;
- (void)_acknowledgmentMousedown:(id)arg1;
- (void)_stickerMouseDown:(id)arg1;
- (void)_selectionChanged:(id)arg1;
- (void)_clearTextSelection:(id)arg1;
- (void)_selectionCleared:(id)arg1;
- (void)_loadMoreHistory;
- (void)quickLookFileTransferGUID:(id)arg1;
- (void)_reloadAttachmentsIfNeeded;
- (void)_invisibleInkMessageClicked:(id)arg1;
- (void)_messageClicked:(id)arg1;
- (void)_dtMessagePaused:(id)arg1;
- (void)_videoMessageVolumeChanged:(id)arg1;
- (void)_videoMessageSeeked:(id)arg1;
- (void)_videoMessageIsNoLongerPlaying:(id)arg1;
- (void)_videoMessageEnded:(id)arg1;
- (void)_videoMessagePaused:(id)arg1;
- (void)_videoMessageStarted:(id)arg1;
- (void)_clearDelayedSelectionForVideo;
- (void)_audioMessageClicked:(id)arg1;
- (id)faceTimeCallHandlerGetChat:(id)arg1;
- (void)faceTimeCallHandler:(id)arg1 callStateChangedForChatItem:(id)arg2;
- (void)_faceTimeInfoButtonClicked:(id)arg1;
- (void)_faceTimeJoinButtonClicked:(id)arg1;
- (void)_playFullScreenEffect:(id)arg1;
@property(readonly, nonatomic) SOFullScreenMomentQueue *fullScreenMomentQueue; // @dynamic fullScreenMomentQueue;
- (void)_didStartPlayingTranscriptEffect:(id)arg1;
- (void)_animationCompleted:(id)arg1;
- (void)_replayClicked:(id)arg1;
- (void)_keepClicked:(id)arg1;
- (void)_augmentDDContext:(id)arg1 withChatItem:(id)arg2;
- (void)_ddResultClicked:(id)arg1;
- (void)_messageDBLClicked:(id)arg1;
- (void)_setUpResponderChain;
- (void)showErrorSheetForExpiredFileTransfer;
- (void)_showMessageErrorDialogForSavedChat;
- (void)_showMessageErrorDialogForMessage:(id)arg1;
- (id)_localizedErrorMessageForIMMessage:(id)arg1;
- (BOOL)_errorShouldOfferRetryForMessage:(id)arg1;
- (void)dateTimeDisplayChanged:(id)arg1;
- (void)_fontStyleChanged:(id)arg1;
- (void)_nameStyleChanged:(id)arg1;
- (BOOL)shouldPinToBottom;
- (unsigned long long)maximumNumberOfMessagesVisibleForHeight:(double)arg1;
- (double)distanceFromBottom;
- (void)_documentFrameChanged:(id)arg1;
- (void)_boundsChanged:(id)arg1;
- (void)_startDynamicLayoutAnimation;
- (void)_liveScrollEnd:(id)arg1;
- (void)_liveScrollStart:(id)arg1;
- (double)_verticalMidPoint;
- (void)animation:(id)arg1 didReachProgressMark:(float)arg2;
- (void)_dynamicPositionTick:(BOOL)arg1;
- (BOOL)lastMessageIsVisible;
- (void)clearUnreadMessagesIfVisible;
- (void)displayResettingKeepMessagesSetting;
- (id)visibleChatItems;
- (id)chatItemRects;
- (struct _NSRange)visibleChatItemRange;
- (id)visibleDOMElementWithBottomOffScreenBuffer;
- (id)visibleDOMElements;
- (id)domElementForChatItem:(id)arg1;
- (void)_watchMessageAddressingDidChange:(id)arg1;
- (void)_applyTextSizeDisplayToTranscriptController;
- (void)_applyTextCustomFontSelected;
- (void)_applyParticipantDisplayToTranscriptStyleController;
- (void)_setupTranscriptStyleController;
- (void)_transcriptSettingsTextSizeDisplayDidChange:(id)arg1;
- (void)_transcriptSettingsCustomFontDidBecomeSelected:(id)arg1;
- (void)_updateTypingIndicatorPosition;
- (void)_updateFont:(id)arg1;
- (void)_updateBackgroundSize;
- (void)_updateDynamicElements;
- (void)_checkTypingIndicator;
- (struct CGPoint)typingIndicatorPosition;
- (void)deleteSelectedMessages;
- (void)clearAllMessages;
- (void)insertAllMessages;
- (void)_unreadCountChanged:(id)arg1;
- (void)_chatControllerDidCompleteDelete:(id)arg1;
- (void)chatItemsDidChange:(id)arg1;
- (void)_handleBusinessNameDidResolve:(id)arg1;
- (void)_handleSearchResultsDidChange:(id)arg1;
- (void)animator:(id)arg1 doStep:(double)arg2 lastStep:(BOOL)arg3;
- (void)scrollToPoint:(struct CGPoint)arg1;
- (void)scrollHelperDidFinishScrolling;
- (id)lastVisibleChatItem;
- (BOOL)scrollToChatItem:(id)arg1 center:(BOOL)arg2;
- (BOOL)scrollToChatItem:(id)arg1;
- (void)scrollToBeginningSmoothly:(BOOL)arg1;
- (void)scrollToEndSmoothly:(BOOL)arg1;
- (void)_scrollToEndWithoutLayoutSmoothly:(BOOL)arg1;
- (void)scrollToPosition:(double)arg1 smoothly:(BOOL)arg2;
- (void)scrollSmoothlyFromPosition:(double)arg1 toPosition:(double)arg2 inTime:(double)arg3;
- (void)scrollFromPosition:(double)arg1 toPosition:(double)arg2 smoothly:(BOOL)arg3;
- (void)_scrollWithoutLayoutFromPosition:(double)arg1 toPosition:(double)arg2 smoothly:(BOOL)arg3;
- (double)savedScrollPosition;
- (double)scrollPositionForDocumentHeight:(double)arg1 clipRect:(struct CGRect)arg2;
- (void)restoreSavedScrollPosition:(double)arg1;
- (void)endLongClick;
- (void)beginLongClick;
- (void)_layoutIfNecessary;
- (void)endChanges;
- (void)beginChanges;
- (void)_changeTimeoutFired:(id)arg1;
- (void)clearHighlightedItems;
- (void)highlightChatItem:(id)arg1;
- (void)highlightSearchString:(id)arg1;
- (void)_loadBaseDocument;
- (void)_finishLoadScripts;
- (void)finishLoad;
- (void)_createTranscriptStyleController;
@property(readonly) ChatController *chatController; // @dynamic chatController;
@property(readonly) FezWebView *webView;
- (void)setInputLineViewController:(id)arg1;
- (void)quickLookTransfer;
- (void)setupFileTransferContextualMenuItems;
- (void)editMessage:(id)arg1;
- (void)downgradeMessage:(id)arg1;
- (id)setupStickerContextualMenuItems;
- (void)setupAcknowledgeContextualMenuItems;
- (void)setupSelectedUnsavedMessageContextualMenuItems;
- (void)setupSelectedMessageContextualMenuItems;
- (BOOL)_unsavedMessageSelected;
- (BOOL)_shouldShowSendAsSMS;
- (void)_initWebView;
- (void)_userScrolled:(id)arg1;
- (void)_loadMoreIfNeeded;
- (double)_boundsLimitForHeight:(double)arg1;
- (void)_reload;
- (void)dealloc;
- (id)initWithCoder:(id)arg1;
- (void)_commonChatTranscriptViewControllerInit;
@property(readonly) struct CGRect visibleContentBounds; // @dynamic visibleContentBounds;
- (void)setBottomOverlap:(double)arg1;
- (double)bottomOverlap;
- (void)setTopOverlap:(double)arg1;
- (double)topOverlap;
- (id)scrollView;
- (void)_updateBodyMarginsWithExtraTop:(double)arg1 extraBottom:(double)arg2;
@property(readonly, nonatomic) BOOL isAnimatingReceipt; // @dynamic isAnimatingReceipt;
@property(readonly, getter=isScrollingToEnd) BOOL scrollingToEnd; // @dynamic scrollingToEnd;
@property(readonly, getter=isScrolling) BOOL scrolling; // @dynamic scrolling;
- (void)_acknowledgmentPickerHelper:(id)arg1 didEndAcknowledgmentForAssociatedMessageGUID:(id)arg2 withAnimation:(BOOL)arg3 completionBlock:(CDUnknownBlockType)arg4;
- (void)acknowledgmentPickerHelper:(id)arg1 didCancelAcknowledgmentForAssociatedMessageGUID:(id)arg2 withAnimation:(BOOL)arg3 completionBlock:(CDUnknownBlockType)arg4;
- (void)acknowledgmentPickerHelper:(id)arg1 didSendAcknowledgment:(long long)arg2 withAnimation:(BOOL)arg3 forAssociatedMessageGUID:(id)arg4 completionBlock:(CDUnknownBlockType)arg5;
- (void)acknowledgmentPickerHelper:(id)arg1 willSendAcknowledgment:(long long)arg2 withAnimation:(BOOL)arg3 forAssociatedMessageGUID:(id)arg4 completionBlock:(CDUnknownBlockType)arg5;
- (void)dismissGroupAcknowledgmentPollWithAnimation:(BOOL)arg1;
- (void)showAcknowledgmentVotesForChatItemGUID:(id)arg1;
- (void)dismissAcknowledgmentPickerWithAnimation:(BOOL)arg1;
- (void)showAcknowledgmentPickerForChatItemGUID:(id)arg1;
- (void)_setUpAcknowledgementHelperForGuid:(id)arg1;
- (id)_pickerElementForChatItem:(id)arg1 withChatItemElement:(id)arg2 attachToElement:(id)arg3;
- (void)dismissAcknowledgmentsWithAnimation:(BOOL)arg1;
- (void)showAcknowledgments:(id)arg1;
- (BOOL)canShowGroupAcknowledgmentPollForChatItemGUID:(id)arg1;
- (BOOL)canShowAcknowledgmentsMenuForChatItemGUID:(id)arg1;
- (id)_buttonForMessageType:(long long)arg1;
- (id)_selectedButtonElement;
- (id)_buttonContainerElement;
- (id)_chatItemElementForID:(id)arg1;
- (id)_chatItemForGUID:(id)arg1;
- (id)_acknowledgmentPickerElement;
- (void)endFullScreenEffectPlayback;
- (void)beginFullScreenEffectPlayback;
- (void)webView:(id)arg1 didScrollDocumentInFrameView:(id)arg2;
- (void)stickerViewerControllerDidClose:(id)arg1;
- (void)stickerViewerControllerWillShow:(id)arg1;
- (void)showStickerViewer:(id)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

